        ...
        <div class="card-body">
            <form name="edit-article" method="post" action="{{ route('articles.update', $article->id) }}" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <textarea rows="3" id="short_description" class="form-control @error('short_description') is-invalid @enderror" name="short_description">{{ old('short_description') ?? $article->short_description  }}</textarea>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <textarea rows="5" id="content" class="form-control text-editor @error('content') is-invalid @enderror" name="content">{{ old('content') ?? $article->content }}</textarea>
                        @error('content')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        @if ($article->cover_image)
                            <img src="{{ asset('storage/uploads/'.$article->cover_image)  }}" class="thumb d-block"/>
                        @endif
                        @error('cover_image')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <div class="form-check form-check-inline  @error('status') is-invalid @enderror">
                            <input class="form-check-input" type="radio" name="status" id="status_0" value="0" @if (old('status') == '0') checked @elseif ($article->status == '0') checked @endif>
                            <label class="form-check-label" for="status_0">Draft</label>
                        </div>
                        <div class="form-check form-check-inline  @error('status') is-invalid @enderror">
                            <input class="form-check-input" type="radio" name="status" id="status_1" value="1" @if (old('status') == '1') checked @elseif ($article->status == '1') checked @endif>
